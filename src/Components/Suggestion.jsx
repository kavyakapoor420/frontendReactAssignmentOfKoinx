...
const Suggestion=()=> {
  ...
  useEffect(() => {
    axios
      ...
      .catch((error) => {
        console.error("not able fetch data from api ", error);
      });
  }, []);
  ...
  return (
    <div className="lg:w-[300px] rounded-2xl p-5 border-2 my-2 mr-2">
      <div className="flex items-center space-x-2">
        <img src={cryptoData.large} alt={cryptoData.name} className="w-6 h-6 rounded-full"
        />
        <span className="text-[16px] font-normal">{cryptoData.name}</span>
        <span className={`text-${cryptoData.data.price_change_percentage_24h.usd > 0
              ? "green"
              : "red"
          }-500 bg-${
      ...
      <img src={cryptoData && cryptoData.sparkline
